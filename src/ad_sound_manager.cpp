...
AdSoundManager::AdSoundManager(const rclcpp::NodeOptions & options)
: Node("ad_sound_manager", options)
{
  sub_state_ = this->create_subscription<autoware_state_machine_msgs::msg::StateMachine>(
    ...
    std::bind(&AdSoundManager::callbackStateMachine, this, std::placeholders::_1)
  );
...
void AdSoundManager::callbackStateMachine(
  const autoware_state_machine_msgs::msg::StateMachine::ConstSharedPtr msg)
{
  RCLCPP_INFO_THROTTLE(
    this->get_logger(),
    *this->get_clock(), 1.0,
    "[AdSoundManager::callbackStateMachine]service_layer_state: %u, control_layer_state: %u",
...
int32_t AdSoundManager::turnIndicatorsToTurnSignal(uint8_t turn_indicators_status) const
{
  using TurnIndicators = autoware_adapi_v1_msgs::msg::TurnIndicators;
  switch (turn_indicators_status) {
    case TurnIndicators::LEFT:
      return tier4_vehicle_msgs::msg::TurnSignal::LEFT;
    case TurnIndicators::RIGHT:
      return tier4_vehicle_msgs::msg::TurnSignal::RIGHT;
    default:
      return tier4_vehicle_msgs::msg::TurnSignal::NONE;
  }
}
...
{
  const int32_t new_turn_signal = turnIndicatorsToTurnSignal(msg->turn_indicators.status);
  if (turn_signal_ != new_turn_signal) {
